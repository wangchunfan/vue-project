<template>
  <div>
              ...
              <li v-for="(item,index) in cartList" :key="'cartList'+index">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascript:;" class="checkbox-btn item-check-btn" @click="editCart('check',item)" v-bind:class="{'check':item.checked == 1}">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" href="javascript:;"  @click="editCart('sub',item)">-</a>
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" @click="delCartConfirm(item.productId)" class="item-edit-btn">
    ...
    <model :mdShow="modelConfim" v-on:close="modelConfim=false">
        <p slot="message">确认删除？</p>
        <div slot="btnGroup" >
            <a class="btn btn--m" href="javascript:;" @click="delCart">确认</a>
            <a class="btn btn--m" href="javascript:;" @click="modelConfim = false">关闭</a>
        </div>
    </model>
  </div>
</template>
...
//   },
  ...
  methods: {
    ...
    //修改商品数量
    editCart(editFlag, item) {
      if (editFlag == "add") {
        item.productNum++;
      } else if (editFlag == "sub") {
        if (item.productNum <= 1) {
          return;
        } else {
          item.productNum--;
        }
      } else if (editFlag == "check") {
